...
test('Kollar så att boken jag gillat syns i listan', async ({ page }) => {
  ...
  await page.getByTestId('star-Hur man tappar bort sin TV-fjärr 10 gånger om dagen').click();
	...
	//gå till sidan där ser sina gillade böcker
	await page.getByTestId('favorites').click();
	
	//Kollar så boken jag gillade förut ligger i listan
	const favorit = page.getByTestId('fav-Hur man tappar bort sin TV-fjärr 10 gånger om dagen');
	await expect(favorit).toBeVisible();
	await expect(favorit).toHaveText('Hur man tappar bort sin TV-fjärr 10 gånger om dagen');
